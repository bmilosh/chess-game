...
class BishopAndRookMovesGetter:
    ...
    def check(
        ...
        if (
            captured
            or safe == False
            or sfr_int + i not in range(8)
            or sff_int + j not in range(8)
        ):
            return False, captured, disc
        occ = self.sog.get_square_occupant(board, sfr_int + i, sff_int + j, opp_col)
        if occ == -1:
            return False, captured, disc
        disc_check = dc.verify_own_king(
            king_position,
            opp_col,
            sff_int,
            sfr_int,
    ...
    def bishop_and_rook_moves(
        self, sfr_int, sff_int, king_position, checking_pieces, board, dc, opp_col, k=1
    ):
        """
