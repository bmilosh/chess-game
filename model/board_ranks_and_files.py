...
class BoardRanks(tk.Frame):
    ...
    def display_ranks(self):
        for x in range(8):
            cell = tk.Label(
                self, bg="#4a390a", text=str(8 - x), font=("Arial", 10, "italic")
            )
            cell.place(relheight=SQUARE_RATIO, relwidth=1.0, rely=x * SQUARE_RATIO)
        ...
        if self.temp_children:
            ...
            for i in range(8):
                ...
                self.children[name].configure(
                    bg="#4a390a", text=str(8 - i), font=("Arial", 10, "italic")
                )
        else:
            self.display_ranks()
...
class BoardFiles(tk.Frame):
    ...
    def display_files(self):
        for x in range(8):
            ...
            cell.place(relheight=1.0, relwidth=SQUARE_RATIO, relx=x * SQUARE_RATIO)
        self.temp_children = self.children
