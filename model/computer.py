...
class Computer:
    ...
    def make_move(
        ...
        for piece in self.ap:
            ...
            print(f"Computer {piece.name} moves: {moves}")
            # print(
            #     f"Opponent has {len(opp_active_pieces)} active pieces: \n{opp_active_pieces}\n"
            # )
            if len(moves):
                shuffle(moves)
                move = moves[0]
                ...
                if (
                    move in [(0, 0), (0, 7), (7, 0), (7, 7)]
                    and hasattr(board[move[0]][move[1]], "can_castle")
                    and board[move[0]][move[1]].colour != self.colour
                ):
