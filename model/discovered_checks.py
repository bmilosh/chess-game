...
class DiscoveredChecks:
    ...
    def verify_own_king(
        ...
        unchk = self.unchecker.uncheck(stf_int, str_int, king_pos, checking_pieces[col])

        dc_b = self.discovered_check_bishop(
            king_pos, opp_col, sff_int, sfr_int, stf_int, str_int, board
        )

        dc_r = self.discovered_check_rook(
            king_pos, opp_col, sff_int, sfr_int, stf_int, str_int, board
        )
    ...
    def verify_opposing_king(
        self, king_pos, col, sff_int, sfr_int, stf_int, str_int, board
    ):
        ...
        dc_b = self.discovered_check_bishop(
            king_pos, col, sff_int, sfr_int, stf_int, str_int, board
        )
        dc_r = self.discovered_check_rook(
            king_pos, col, sff_int, sfr_int, stf_int, str_int, board
        )
    ...
    def discovered_check_bishop(
        ...
        if abs(sfr_int - king_rank) == abs(sff_int - king_file):
            ...
            for r, f in zip(
                range(king_rank + r_dir, end_rank, r_dir),
                range(king_file + f_dir, end_file, f_dir),
            ):
                if 0 <= r <= 7 and 0 <= f <= 7:
