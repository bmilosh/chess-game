...
class Pawn(Piece):
    ...
    # def get_legal_moves(self, p: Pawn, kings_positions: list[tuple], checking_pieces: dict, flipped=False) -> list:
    #     def get_occ(r, f):
...
#     def get_legal_moves(self, kings_positions: list[tuple], checking_pieces: dict, board):
#         self.legal_moves = []
#         opp_col = "white" if self.colour == "black" else "black"
...
class Queen(Piece):
    ...
    def move(self, square_from: str, square_to: str, kings_positions: list[tuple],
        ...
        # bishop.rank, bishop.file = self.rank, self.file
        # rook.rank, rook.file = self.rank, self.file
        # self.bishop.rank, self.bishop.file = self.rank, self.file
        # self.rook.rank, self.rook.file = self.rank, self.file
        stf_int, str_int = convert_to_int(square_to)
        bishop_check = self.bishop.move(square_from, square_to, kings_positions, king_under_check,
                                   board, sqv, queen_move=self.rook, checking_pieces=checking_pieces)
        ...
        if bishop_check:
            self.rank, self.file = self.bishop.rank, self.bishop.file
            print("valid bishop-like move from the queen")
            return bishop_check
        rook_check = self.rook.move(square_from, square_to, kings_positions, king_under_check,
                         board, sqv, queen_move=self.bishop, checking_pieces=checking_pieces)
        if rook_check:
            print("valid rook-like move from the queen")
            self.rank, self.file = self.rook.rank, self.rook.file
        return rook_check
    ...
    #                                board, sqv, queen_move=rook, checking_pieces=checking_pieces)
    #     # rook_check = rook.move(square_from, square_to, kings_positions, king_under_check,
    #     #                        board, sqv, queen_move=True, checking_pieces=checking_pieces)
    #     # return bishop_check or rook_check
    #     if bishop_check:
    ...
    #         return bishop_check
    #     rook_check = rook.move(square_from, square_to, kings_positions, king_under_check,
    #                      board, sqv, queen_move=bishop, checking_pieces=checking_pieces)
    #     if rook_check:
    #         print("valid rook-like move from the queen")
    ...
    def __check(self, str_int, stf_int, board, threat, opp_col, k=1):
        ...
        def check(safe, i, j):
            if 0 <= str_int + i <= 7 and 0 <= stf_int + j <= 7 and \
                ...
                print(f"{type(p)=} {threat=} {opp_col=} {p.colour=} {str_int + i=} {stf_int + j=}")
